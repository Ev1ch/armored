...
namespace Core.Parallax
{
    public class InfiniteParallaxLayer
    {
        ...
        private void FixLayersPositions(float targetPosition)
        {
            ...
            var relativePosition = activeLayer.position.x;
            var direction = relativePosition > targetPosition ? -1 : 1;

            if (layerToMove.position.x > relativePosition && direction > 0 || layerToMove.position.x < relativePosition && direction < 0)
            {
                return;
            }

            layerToMove.position = new Vector3(relativePosition + _layerHorizontalSize * direction, 0, 0);
        }

        private bool IsLayerActive(Transform layer, float targetPosition) => Mathf.Abs(layer.position.x - targetPosition) <= _layerHorizontalSize / 2;
    }
}